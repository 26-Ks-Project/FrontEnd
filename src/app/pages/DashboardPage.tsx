...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setHumidity(prev => Math.max(40, Math.min(90, +(prev + (Math.random() - 0.5) * 2).toFixed(0))));
      setSoilMoisture(prev => Math.max(30, Math.min(100, +(prev + (Math.random() - 0.5) * 1.5).toFixed(0))));
      setLightIntensity(prev => Math.max(0, Math.min(1200, +(prev + (Math.random() - 0.5) * 50).toFixed(0))));
